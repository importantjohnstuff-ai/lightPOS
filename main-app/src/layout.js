...
export function renderSidebar() {
    ...
    const menuItems = [
        ...
        { label: "Floorplan", icon: "🗺️", hash: "#inventory-spatial", permission: "items", type: "read" },
        ...
        { label: "Reports v2", icon: "📈", url: "../Reports-module/index.php", permission: "reports", type: "read" },
    ...
    menuItems.forEach(item => {
        if (item.section) {
            ...
            if (!item.permission || checkPermission(item.permission, item.type)) {
                const targetUrl = item.url || obfuscate(item.hash);
                const isCurrent = item.hash && currentHash === item.hash;
                const activeClass = isCurrent ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900";
                ...
                if (isCollapsed) {
                    html += `<a href="${targetUrl}" class="relative flex justify-center py-3 my-1 mx-2 rounded-lg transition-colors duration-150 ${activeClass}" title="${item.label}"><span class="text-xl">${item.icon}</span>${warningBadge}</a>`;
                } else {
                    const activeBorder = isCurrent ? "border-r-4 border-blue-700" : "";
                    html += `<a href="${targetUrl}" class="relative flex items-center px-4 py-2 text-sm font-medium transition-colors duration-150 ${activeClass} ${activeBorder}"><span class="mr-3 text-lg">${item.icon}</span>${item.label}${warningBadge}</a>`;
                }
            }
        }
    });
    ...
    headerActions.innerHTML = `
        ...
        ${canSeeNotifications ? `
            <div class="relative">
                ...
                <div id="notification-dropdown" class="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl overflow-hidden z-[9999] hidden border border-blue-200 text-gray-800">
